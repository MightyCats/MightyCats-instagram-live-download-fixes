...
export default class LiveVideoRecorder extends EventTarget {
  ...
  async initializeWithMpdUrl(mpdUrl) {
    ...
    // const videoSegment = mpdXml.getElementById('dash-lp-pst-v').getElementsByTagName('SegmentTemplate')[0]
    // 変更 start
    let videoSegment
    let videoElement = mpdXml.getElementById('dash-lp-hd2-v')
    if (videoElement != null) {
      videoSegment = videoElement.getElementsByTagName('SegmentTemplate')[0]
    }
    else {
      videoElement = mpdXml.getElementById('dash-lp-pst-v')
      if (videoElement != null) {
        videoSegment = videoElement.getElementsByTagName('SegmentTemplate')[0]
      }
      else {
        videoElement = mpdXml.getElementById('dash-lp-hd1-v')
        if (videoElement != null) {
          videoSegment = videoElement.getElementsByTagName('SegmentTemplate')[0]
        }
        else {
          videoElement = mpdXml.getElementById('dash-lp-hd-v')
          if (videoElement != null) {
            videoSegment = videoElement.getElementsByTagName('SegmentTemplate')[0]
          }
          else {
  ...
  async startRecording(verbose) {
    ...
    while (!this.shouldStop && !this.streamIsOver) {
      ...
      // const timelineParts = mpdXml.getElementById('dash-lp-pst-v').getElementsByTagName('S')
      // 変更 start
      let timelineParts
      let timelineElement = mpdXml.getElementById('dash-lp-hd2-v')
      if (timelineElement != null) {
        timelineParts = timelineElement.getElementsByTagName('S')
      }
      else {
        timelineElement = mpdXml.getElementById('dash-lp-pst-v')
        if (timelineElement != null) {
          timelineParts = timelineElement.getElementsByTagName('S')
        }
        else {
          timelineElement = mpdXml.getElementById('dash-lp-hd1-v')
          if (timelineElement != null) {
            timelineParts = timelineElement.getElementsByTagName('S')
          }
          else {
            timelineElement = mpdXml.getElementById('dash-lp-hd-v')
            if (timelineElement != null) {
              timelineParts = timelineElement.getElementsByTagName('S')
            }
            else {
              return {
                status: 'fail',
                message: 'Timeline element is null',
              }    
            }  
          }
        }
